...
For new Markdown image content, use the site's framed figure style rather than a bare image. Wrap images in a `figure` with a caption below it, preserving the thin border and consistent inner spacing defined in `src/styles/markdown.css`. For two related images, use the existing `image-comparison` structure: square, center-cropped images arranged symmetrically, with equal spacing to the outer border and the central divider. Keep the original image files intact and use CSS cropping with `object-position: center` when equal display dimensions are needed.
...
For every future code change, create and push a preparatory Git commit before editing (use an allowed empty commit when no file change exists yet) with a clear `chore: prepare ...` message describing the planned work. After the implementation and validation are complete, create and push a second Conventional Commit that explicitly summarizes the completed changes. Do not stage generated caches, dependency stores, or unrelated user changes.
